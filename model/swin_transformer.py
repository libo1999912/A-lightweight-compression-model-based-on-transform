...
class SwinTransformerV2Cr(nn.Module):
    ...
    def __init__(
        ...
        # scale
        # conv = nn.Conv2d(in_channels = in_chans, out_channels = out_chans, kernel_size = 3, padding=(3 // 2), bias=True)
        conv = default_conv
        modules_tail = [
            Upsampler(conv, scale=4, n_feats=128, bn=False, act=False, bias=True),
            # self.outc
        ]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, T, C, H, W = x.shape  #获取输入张量的形状
        x = x.view(B * T, C, H, W)  # 将张量从 (B, T, C, H, W) 调整为 (B * T, C, H, W)
        if self.residual:
            skip = x
        else:
            skip = torch.zeros_like(x)
        x = self.forward_features(x)  #将调整形状后的张量传递给 forward_features 方法
        x = self.forward_head(x)  # 将输出传递给 forward_head 方法
        x = x + skip[:, :self.out_chans, :, :]
        # x = self.tail(x)
        #return x.view(B, T, self.out_chans, H, W)  #将张量恢复为 (B, T, self.out_chans, H, W)
        # 使用插值方法将时间步数从 5 调整为 10
        x = x.view(B, T, self.out_chans, H, W)
        x = x.permute(0, 2, 1, 3, 4)  # 将张量从 (B, T, C, H, W) 调整为 (B, C, T, H, W)
        x = F.interpolate(x, size=(10, H, W), mode='trilinear', align_corners=False)
        x = x.permute(0, 2, 1, 3, 4)  # 将张量恢复为 (B, T, C, H, W)
...
if __name__ == "__main__":
    ...
    dumpy_input = torch.randint(0, 1, (8,1, 3, 76, 128)).float()

    model = SwinTransformerV2Cr(img_size=(76, 128), patch_size=1, in_chans=3, depths = (2, 2, 6, 2), num_heads = (3, 6, 12, 24), window_size = (4, 8))
    print(model)
    p = model(dumpy_input)
    print(p.shape)
